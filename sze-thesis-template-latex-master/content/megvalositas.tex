...
\section{Megvalósításhoz használt osztályok és struktúrák}
...
\par A \lstinline{BallMovement} osztályhoz hasonlóan létezik egy ún. \textbf{\lstinline{BallData}} osztály, amely arra szolgál, hogy a golyókon megvizsgált vizsgálati szempontokat eltárolja és kiszámolja. Ilyen szempontok a \ref{section:analyze_gameplay} részben említett elemzési szempontok, ezek közül a teljes megtett távolságot a \lstinline{totalDistance}, az útvonalat a \lstinline{path}, a pillanatnyi sebességeket a \lstinline{speed} és az átlagsebességet a \lstinline{averageSpeed} változók tárolják. Az osztály része még a \lstinline{previousUpdateFrame} változó, ez eltárolja az adott golyóhoz a legutóbbi frissítés időpontját képkocka formájában, amelyet az \lstinline{addPosition} függvény felhasznál a két frissítés közt eltelt idő megállapítására, továbbáa az előzőekben említett változókat is beállítja vagy frissíti. A \lstinline{sumOfSpeeds} változó az átlagsebesség kiszámolásához tárolja a sebességek összegét. A \textbf{\lstinline{BallData}} osztály felépítése a \ref{fig:balldata_class} ábrán látható.
...
\begin{figure}[!ht]
    ...
    \caption{Mintaillesztések eredményei confusion mátrixban.}
...
\begin{figure}[!ht]
    ...
    \caption{Neurális hálózattal történt becslés eredményei confusion mátrixban.}
...
\section{Játékmenet vizsgálati szempontok}
...
\par A \lstinline{distance} értékének kiszámolása után az eltelt idő nagysága az eddigi (\lstinline{previousUpdateFrame}) és a jelenlegi (\lstinline{framePosition}) képkockák számát tároló változók különbségéből adódik. A \lstinline{time} változó ezt az értéket ugyancsak képkockák számának formájában tárolja. A sebesség kiszámolásához a távolság osztásra kerül az eltelt idővel, kiszámolt sebesség a \lstinline{currentSpeed} változóban kerül tárolásra, amelynek értéke a \lstinline{speed} tömbhöz a későbbiekben hozzáadódik. Ez a művelet a kódrészlet 16. sorában látható. A sebességek tömbjét növelő sor után a kódban következik az eltárolt útvonalak növelése, amely a 17. sorban a \lstinline{path} tömb \lstinline{newPosition} értékkel való növelését teszi meg. Az elmozdulás és pillanatyni sebesség kiszámolása utáni kódsorok a golyó átlagsebességét számoják, úgy, hogy a sebességek összege a \lstinline{sumOfSpeeds} változóban tárolódik, amelyhez hozzáadásra kerül a jelenlegi sebesség. Ezután az átlagsebesség a sebességek összegéből és a tárolt sebességek lista méretének hányadosából kapható meg. Az átlagsebesség kiszámolása a 19. - 20. sorokban látható.
